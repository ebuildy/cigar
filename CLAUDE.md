...
- Authenticate via `WEBHOOK_AUTH_METHOD`, a single method (`secret` or `signing_token`; default `secret`). `secret`: constant-time compare of `X-Gitlab-Token` against `WEBHOOK_SECRET` (an empty configured secret never authenticates). `signing_token`: GitLab signing token — verify the `webhook-signature` HMAC-SHA256 over `{webhook-id}.{webhook-timestamp}.{body}` using `WEBHOOK_SIGNING_TOKEN`, which must be `whsec_<standard base64 of a 32-byte key>` (`echo "whsec_$(openssl rand -base64 32)"`) and identical to the `signing_token` on each project hook; rejecting timestamps outside a 5m window (replay protection). `webhook.NewApp` takes one `Authenticator`; an unauthenticated request → `401`, no body detail.
...
Helm chart in `deploy/chart/cigar`: Deployment (2 replicas, PDB), Service (http 8080 + ops 8081), Ingress (TLS), NetworkPolicy (ingress limited to the two ports; egress only DNS + GitLab + Prometheus), resources set (practice what we preach), `runAsNonRoot` uid 65532, read-only rootfs, seccomp `RuntimeDefault`, no SA token automount. Env vars map to `config.*` values. Secrets are configured per env var under `secrets.<ENV_VAR>` (`GITLAB_TOKEN`, `WEBHOOK_SECRET`, `WEBHOOK_SIGNING_TOKEN`, `COMMANDS_SIGNING_KEY`), each with exactly one source: a literal `value` (collected into one chart-owned Secret), an `existingSecret` you manage (optional `key:`, defaults to the env var name), or an `externalSecret` naming an entry in the top-level `externalSecrets` map. That map holds **raw ExternalSecret specs**, rendered verbatim as `<fullname>-<key>` (ESO defaults the managed Secret to the same name, which is what the Deployment reads) — the chart models no ESO internals, so stores/generators/dataFrom/templates/rewrites all work without chart support. `externalSecretsGenerator` likewise creates generators: an entry is the `passwordSpec` of a Password `ClusterGenerator` (names verbatim, since specs reference them by `sourceRef.generatorRef.name`; `allowRepeat`/`noUpper` defaulted as the CRD requires), or a raw `spec` for any other kind. `externalSecretApiVersion` switches v1/v1beta1. `_helpers.tpl` owns the rest: `cigar.requiredSecretNames` (config-driven — only those are injected or rendered), `cigar.secretRefName`/`cigar.secretRefKey`, `cigar.externalSecretName`, and `cigar.validateSecrets` (included by deployment/secret/externalsecret), which rejects an unknown `secrets.*` key, a required secret with none or several sources, an `externalSecret` naming no declared entry, and a literal `WEBHOOK_SIGNING_TOKEN` that isn't `whsec_<base64 of 32 bytes>`. There is **no default source**, so bare `helm lint`/`helm template` need placeholder `--set secrets.GITLAB_TOKEN.value=x` (the mise tasks do). Image: `ghcr.io/ebuildy/cigar`, tag defaults to `appVersion`. Validate chart changes with `mise r helm:test` (`helm lint` + the helm-unittest suites in `deploy/chart/cigar/tests/`, excluded from the package via `.helmignore`); IDE YAML errors on Go template syntax are noise. The dev stack installs the External Secrets Operator plus `ClusterGenerator`s from `.dev/charts/external-secrets-config` (`cigar-password`, `cigar-fake`), and the dev cigar release declares a `cigar-keys` Password generator (`secretKeys`: `WEBHOOK_SIGNING_TOKEN`, `COMMANDS_SIGNING_KEY`) plus a `keys` ExternalSecret pulling both with `dataFrom` at `refreshInterval: "0s"` (fetch-once, so they never rotate out from under GitLab; ESO persists the result in a `GeneratorState`), with the `whsec_` template + `mergePolicy: Merge`. Only `GITLAB_TOKEN` stays in the script-written `cigar-secrets`, since gitlab-rails is the only thing that can issue a PAT. `deploy-cigar.sh` therefore *reads* the generated signing token back out of the `cigar-keys` Secret after deploying and registers that on every project hook — it no longer mints one. Chart reference doc: `docs/deploy.md`.
